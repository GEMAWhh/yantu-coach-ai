...
- 云端个人版优先，保留本地开发与离线核心能力；
- 当前仍为单用户边界，多用户能力必须另行设计身份与数据隔离；
- 确定性规则核心与 AI 能力隔离；
- 结构化数据与原始文件分离；
- 所有自动变化可追溯；
- 先模块化单体，不提前微服务；
...
云端演进按 `ADR-0004` 分阶段进行。第一阶段只建立前端到独立 API 的安全连接边界；认证、云数据库和对象存储不得未经独立迁移设计直接替换。
...
云端个人版部署保持相同的模块边界：静态 Web 与 FastAPI 独立部署，生产浏览器通过 Sites Worker 的同源 `/health` 和 `/api/*` 路径连接固定 Render API；本地或其他托管仍可通过 `VITE_YANTU_API_BASE_URL` 指定 API Origin。后端 CORS 继续只允许已批准的 Web Origin。
